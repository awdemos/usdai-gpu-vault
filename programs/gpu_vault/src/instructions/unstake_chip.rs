...
pub fn handler(ctx: Context<UnstakeChip>, amount: u64) -> Result<()> {
    require!(!ctx.accounts.vault_config.paused, VaultError::Paused);
    require!(amount > 0, VaultError::ZeroAmount);

    usd_ai_stake::unstake_cpi(
        &ctx.accounts.usd_ai_stake,
        usd_ai_stake::UnstakeAccounts {
            staker: ctx.accounts.owner.to_account_info(),
            s_chip_source: ctx.accounts.owner_schip_account.to_account_info(),
            chip_destination: ctx.accounts.owner_chip_account.to_account_info(),
